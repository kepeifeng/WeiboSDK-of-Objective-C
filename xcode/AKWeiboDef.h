...
@interface AKUserTaskInfo : NSObject
...
@interface AKBasicInfo : NSObject

    @property NSString * id; //用户UID
    @property NSString * name; //用户名
    @property NSString * screen_name; //用户昵称
    @property NSString * city; //所在城市代码ID
    @property NSString * province; //所在省份代码ID
    @property NSString * location; //所在地信息
    @property NSString * created_at; //用户创建时间
    @property NSString * description; //用户描述信息
    @property NSString * domain; //个性化域名
    @property NSString * gender; //性别，m：男、f：女
    @property NSString * credentials_num; //证件号码
    @property NSString * credentials_type; //证件类型
    @property NSString * lang; //语言设置
    @property NSString * profile_image_url; //头像地址
    @property NSString * birthday; //生日信息
    @property NSString * birthday_visible; //生日隐私类型，0：保密、1：只显示月日、2：只显示星座、3：所有人可见
    @property NSString * email; //联系邮箱地址
    @property NSString * email_visible; //邮箱地址隐私类型，0：自己可见、1：我关注人可见、2：所有人可见
    @property NSString * msn; //账号信息
    @property NSString * msn_visible; //账号隐私类型，0：自己可见、1：我关注人可见、2：所有人可见
    @property NSString * qq; //号码
    @property NSString * qq_visible; //号码隐私类型，0：自己可见、1：我关注人可见、2：所有人可见
    @property NSString * real_name; //真实姓名
    @property NSString * real_name_visible; //真实姓名隐私类型，0：自己可见、1：我关注人可见、2：所有人可见
    @property NSString * url_string; //用户博客地址
    @property NSString * url_visible; //用户博客地址隐私类型，0：自己可见、1：我关注人可见、2：所有人可见

@end

@interface AKEducationInfo : NSObject

    @property NSString * id; //学校ID
    @property NSString * type;//学校类型，1：大学、2：高中、3：中专技校、4：初中、5：小学
    @property NSString * school;//学校名称
    @property NSString * department;//学校院系名称
    @property NSString * year;//入学年份
    @property NSString * visible;//可见设置类型，0：自己可见、1：我关注人可见、2：所有人可见

@end

@interface AKCareerInfo : NSObject

    @property NSString * id;//公司ID
    @property NSString * city;//城市代码ID
    @property NSString * province;//省份代码ID
    @property NSString * start;//入职时间
    @property NSString * end;//离职时间
    @property NSString * company;//公司名称
    @property NSString * department;//公司部门名称
    @property NSString * visible;//可见设置类型，0：自己可见、1：我关注人可见、2：所有人可见
